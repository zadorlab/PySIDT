    ...
    for i, atm1 in enumerate(grp.atoms):
        ...
        if not skip and atm1.reg_dim_atm[1] != [] and set(atm1.reg_dim_atm[1]) != set(atm1.atomtype):
            ...
            if len(atyp) == 1 and atyp[0] in Rx:
                ...
                if all([set(child.group.atoms[i].atomtype) <= set(vals) for child in node.children]):
                    if not test:
                        atm1.atomtype = vals
                    else:
        ...
        if not skip and atm1.reg_dim_u[1] != [] and set(atm1.reg_dim_u[1]) != set(atm1.radical_electrons):
            if len(atm1.radical_electrons) == 1:
                ...
                if all([set(child.group.atoms[i].radical_electrons) <= set(vals)
                        if child.group.atoms[i].radical_electrons != [] else False for child in node.children]):
                    if not test:
        ...
        if not skip and atm1.reg_dim_site[1] != [] and set(atm1.reg_dim_site[1]) != set(atm1.site):
            if len(atm1.site) == 1:
                ...
                if all([set(child.group.atoms[i].site) <= set(vals)
                        if child.group.atoms[i].site != [] else False for child in node.children]):
                    if not test:
                        atm1.site = vals
                    else:
                        oldvals = atm1.site
        ...
        if not skip and atm1.reg_dim_morphology[1] != [] and set(atm1.reg_dim_morphology[1]) != set(atm1.morphology):
            if len(atm1.morphology) == 1:
                ...
                if all([set(child.group.atoms[i].morphology) <= set(vals)
                        if child.group.atoms[i].morphology != [] else False for child in node.children]):
                    if not test:
        ...
        if (not skip and atm1.reg_dim_r[1] != [] and
            ...
            if 'inRing' not in atm1.props.keys():
                if (all(['inRing' in child.group.atoms[i].props.keys() for child in node.children]) and
                        all([child.group.atoms[i].props['inRing'] == atm1.reg_dim_r[1] for child in node.children])):
                    if not test:
            ...
            for j, atm2 in enumerate(grp.atoms[:i]):
                ...
                if grp.has_bond(atm1, atm2):
                    ...
                    if len(bd.order) == 1:
                        ...
                        if vals != [] and all([set(child.group.get_bond(child.group.atoms[i], child.group.atoms[j]).order) <= set(vals) for child in node.children]):
                            if not test:
                                bd.order = vals
                            else:
                                oldvals = bd.order
