...
@pytest.fixture
def old_tree_path():
    return Path(__file__).parent / "data" / "old_tree.json"
...
def test_read_old_tree(old_tree_path):
    nodes = read_nodes(old_tree_path)
    assert nodes["root"].rule.value == 2
    assert nodes["root"].children[0].rule.value == 1